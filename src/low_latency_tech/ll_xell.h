...
class XeLL : public virtual LowLatencyTech {
    ...
    std::array<std::atomic<std::uint32_t>, 64> sent_sleep_frame_ids_{};
    uint64_t simulation_start_last_id {};
    uint64_t sleep_last_id {};
    std::atomic<bool> exact_frame_pacing_{false};
    bool last_sleep_mode_valid_ = false;
    std::uint32_t last_low_latency_mode_ = 0;
    std::uint32_t last_minimum_interval_us_ = 0;
    std::uint32_t last_low_latency_boost_ = 0;
...
public:
    ...
    void sleep() override;
    void sleep_with_frame_id(uint64_t frame_id) override;
