...
namespace fakenvapi {
    ...
    NvAPI_Status __cdecl NvAPI_Initialize() {
        ...
        // Retry frontend hook installation after the application has had a
        // chance to load its low-latency SDK/driver modules.
        StreamlineCapability::Init();
        fakexell::Init();
        AL2Proxy::hookAntiLag();
        AudioHooks::initialize();
    ...
    NvAPI_Status __cdecl NvAPI_D3D12_NotifyOutOfBandCommandQueue(ID3D12CommandQueue* pCommandQueue, NV_OUT_OF_BAND_CQ_TYPE cqType) {
        ...
        if (!pCommandQueue)
            return ERROR_VALUE(NVAPI_INVALID_ARGUMENT);
        return LowLatencyCtx::get()->NotifyOutOfBandCommandQueue(pCommandQueue);
    }
    ...
    NvAPI_Status __cdecl NvAPI_Vulkan_InitLowLatencyDevice(HANDLE vkDevice, HANDLE *signalSemaphoreHandle) {
        ...
        if (!VulkanDeviceRegistry::get_state(device, &state)) {
            // In Vulkan auto-coexistence mode another proxy (for example
            // OptiScaler) owns the Vulkan hook chain, so this VkDevice is
            // intentionally absent from our registry. Do not manufacture a
            // timeline semaphore for an unowned device. Preserve the legacy
            // failure contract used by the pre-r2 path: callers that already
            // tolerate NVAPI_ERROR continue down the same fallback path, while
            // we avoid the invalid vkCreateSemaphore attempt and its -3 log.
            *signalSemaphoreHandle = nullptr;
            return NVAPI_ERROR;
        }

        VkSemaphore semaphore = VK_NULL_HANDLE;
        const VkResult result = VulkanDeviceRegistry::ensure_low_latency_semaphore(device, &semaphore);
        if (result != VK_SUCCESS) {
            spdlog::error("Failed to create Vulkan low-latency timeline semaphore: {}", static_cast<int>(result));
            return NVAPI_ERROR;
        }

        *signalSemaphoreHandle = reinterpret_cast<HANDLE>(semaphore);
        return OK();
    }
    ...
    NvAPI_Status __cdecl NvAPI_Vulkan_Sleep(HANDLE vkDevice, NvU64 signalValue) {
        ...
        const auto device = reinterpret_cast<VkDevice>(vkDevice);
        const VkResult signal_result = VulkanDeviceRegistry::signal_low_latency_semaphore(device, signalValue);
        if (signal_result != VK_SUCCESS) {
            spdlog::debug("Vulkan low-latency semaphore signal skipped/failed: {}", static_cast<int>(signal_result));
        }
    ...
    NvAPI_Status __cdecl Fake_InformPresentFG(bool frame_interpolated, uint64_t reflex_frame_id) {
        LowLatencyCtx::get()->FrontendSetFgType(
            policy::InputFrontend::Reflex, frame_interpolated, reflex_frame_id);
        return OK();
    }
    ...
    NvAPI_Status __cdecl Fake_GetLowLatencyCtx(void** low_latency_context, Mode* mode) {
        ...
        // OptiScaler calls this after its optional low-latency DLLs are loaded,
        // making it a reliable late-load retry point for both translated
        // frontends.  Both hook installers are idempotent.
        StreamlineCapability::Init();
        fakexell::Init();
        AL2Proxy::hookAntiLag();
        AudioHooks::initialize();
