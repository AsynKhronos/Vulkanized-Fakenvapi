...
public:
  ...
  // Rebase the estimator across a known external/structural stall. This keeps
  // all LatencyFleX timestamps in the real clock domain while collapsing only
  // the supplied pause interval. References before the interval advance by the
  // full duration, references inside it advance to stall_end_timestamp, and
  // references after it are untouched.
  void CompensateExternalStall(uint64_t duration, uint64_t stall_start_timestamp,
                               uint64_t stall_end_timestamp) {
    if (duration == 0 || stall_end_timestamp <= stall_start_timestamp) {
      return;
    }

    auto compensate_reference = [&](uint64_t &timestamp) {
      if (timestamp >= stall_end_timestamp) {
        return;
      }
      if (timestamp <= stall_start_timestamp) {
        timestamp += duration;
      } else {
        timestamp = stall_end_timestamp;
      }
    };

    for (std::size_t i = 0; i < kMaxInflightFrames; ++i) {
      if (frame_begin_ids_[i] != UINT64_MAX) {
        compensate_reference(frame_begin_ts_[i]);
      }
    }

    if (prev_frame_end_id_ != UINT64_MAX) {
      compensate_reference(prev_frame_end_ts_);
      compensate_reference(prev_frame_real_end_ts_);
    }
    if (frame_end_projection_base_ != UINT64_MAX) {
      compensate_reference(frame_end_projection_base_);
    }
  }
